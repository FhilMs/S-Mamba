...
def antispiral_gather(tensor):
    # 逆时针螺旋式扫描,返回拼接结果
    B, C, H, W = tensor.size()
    result = torch.zeros(B, C, H * W, device=tensor.device, dtype=tensor.dtype)
    idx = 0
    x, y = 0, 0
    dx, dy = 1, 0
    visited = set()
...
def spiral_gather(tensor):
    ...
    return result
def spiral_scatter(tensor_flat, original_shape):
    # 把顺时针螺旋元素拼接起来的一维向量还原为最初的矩阵形式
    B, C, H, W = original_shape
    result = torch.zeros(B, C, H, W, device=tensor_flat.device, dtype=tensor_flat.dtype)
    ...
    return result
def antispiral_scatter(tensor_flat, original_shape):
    # 把逆时针螺旋元素拼接起来的一维向量还原为最初的矩阵形式
    B, C, H, W = original_shape
    result = torch.zeros(B, C, H, W, device=tensor_flat.device, dtype=tensor_flat.dtype)
...
class CrossScan(torch.autograd.Function):
    #  这里是把图像按照特定方向展平的地方，改变扫描方向可以在这里修改
    @staticmethod
    def forward(ctx, x: torch.Tensor):
        ...
        # 提供斜向和反斜向的扫描
        xs[:, 4] = spiral_gather(x)
        xs[:, 5] = antispiral_gather(x)
    ...
    def backward(ctx, ys: torch.Tensor):
        ...
        y_da = spiral_scatter(y_da[:, 0], (B, C, H, W)) + antispiral_scatter(y_da[:, 1], (B, C, H, W))
...
class CrossMerge(torch.autograd.Function):
    ...
    def forward(ctx, ys: torch.Tensor):
        ...
        # 把斜向和反斜向的部分都转成原来的最初的矩阵形式，再相加
        y_da = spiral_scatter(y_da[:, 0], (B, D, H, W)) + antispiral_scatter(y_da[:, 1], (B, D, H, W))
    ...
    def backward(ctx, x: torch.Tensor):
        ...
        # 提供斜向和反斜向的扫描
        xs[:, 4] = spiral_gather(x.view(B, C, H, W))
        xs[:, 5] = antispiral_gather(x.view(B, C, H, W))
        xs[:, 6:8] = torch.flip(xs[:, 4:6], dims=[-1])
...
class CrossScan_Ab_diagonal(torch.autograd.Function):
    #  这里是把图像按照特定方向展平的地方，改变扫描方向可以在这里修改
    @staticmethod
    def forward(ctx, x: torch.Tensor):
        B, C, H, W = x.shape
...
class CrossMerge_Ab_diagonal(torch.autograd.Function):
    @staticmethod
    def forward(ctx, ys: torch.Tensor):
        B, K, D, H, W = ys.shape
        ctx.shape = (H, W)
...
# =============
#  这里是mamba的具体内容，要增加扫描方向就在标记FhilMs处改
...
class OSSM(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor, to_dtype=False, channel_first=False):
        ...
        #  这里进行data expand操作，也就是把相同的数据在不同方向展开成一维，并拼接起来,但是这个函数只用在旧版本
        # 把横向和竖向拼接在K维度
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers):
            ...
            downsample = _make_downsample(
                ...
                self.dims[i_layer], 
                norm_layer=norm_layer,
            ) if (i_layer != 0) else nn.Identity()  #  修改为i_layer != 0，也就是第一层不下采样，和论文的图保持一致，也方便我取出每个尺度处理好的特征
    ...
    def _make_layer(
        ...
        **kwargs,
        ...
        return nn.Sequential(OrderedDict(
            #  把downsample放到前面来，方便我取出encoder中每个尺度处理好的图像，而不是刚刚下采样完的图像
            downsample=downsample,
